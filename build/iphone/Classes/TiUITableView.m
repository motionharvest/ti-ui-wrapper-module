...
-(void)frameSizeChanged:(CGRect)frame bounds:(CGRect)bounds
{
	...
	// Objective-C can suck it
	if (tableFooterPullView!=nil)
	{
		tableFooterPullView.frame = CGRectMake(0.0f, self.tableView.contentSize.height, self.tableView.bounds.size.width, self.tableView.bounds.size.height);
    ...
    // Since the header proxy is not properly attached to a view proxy in the _tiuicommonjsmodulewrapper
...
-(void)setFooterPullView_:(id)value
{
	...
	{
		tableFooterPullView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f - self.tableView.bounds.size.height, self.tableView.bounds.size.width, self.tableView.bounds.size.height)];
		tableFooterPullView.backgroundColor = [UIColor lightGrayColor];
		UIView *view = [value view];
