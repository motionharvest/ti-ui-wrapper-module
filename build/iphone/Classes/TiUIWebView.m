...
static NSString * const kTiUICommonJSModuleWrapperJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
-(NSString*)_tiuicommonjsmodulewrapperInjection
{
	...
	[html appendString:kTiUICommonJSModuleWrapperJavascript];
...
{
	// attempt to make well-formed HTML and inject in our TiUICommonJSModuleWrapper bridge code
	...
	if (range.location!=NSNotFound)
	{
		...
		if (nextRange.location!=NSNotFound)
		{
			...
			[html appendString:[self _tiuicommonjsmodulewrapperInjection]];
	...
	else
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:path])
		{
			...
			else
			{
				...
				{
					NSLog(@"[WARN] I have no idea what the appropriate text encoding is for: %@. Please report this to TiUICommonJSModuleWrapper support.",url);
				}
			}
